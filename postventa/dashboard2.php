...
foreach ($solicitudes as $solicitud) {
    ...
    if ($estado === 'pendiente')
        $porProyecto[$proyecto]['pendientes']++;
    if ($estado === 'aprobado')
        $porProyecto[$proyecto]['en_gestion']++;
    if ($estado === 'resuelto')
        $porProyecto[$proyecto]['resueltos']++;
    if ($estado === 'no_corresponde')
        $porProyecto[$proyecto]['rechazados']++;
...
$solicitudesExport = array();
foreach ($solicitudes as $s) {
    $solicitudesExport[] = array(
        'id' => 'PC-' . date('Y', strtotime($s['created_at'])) . '-' . str_pad($s['id'], 3, '0', STR_PAD_LEFT),
        'id_num' => (int)$s['id'],
        'fecha' => date('d/m/Y', strtotime($s['created_at'])),
        'rut' => isset($s['rut']) ? $s['rut'] : '',
        'nombre' => isset($s['nombre']) ? $s['nombre'] : '',
        'email' => isset($s['email']) ? $s['email'] : '',
        'telefono' => isset($s['telefono']) ? $s['telefono'] : '',
        'rol' => (isset($s['rol_solicitante']) && $s['rol_solicitante'] === 'administrador_edificio') ? 'Administrador' : 'Propietario',
        'proyecto' => isset($s['obra_nombre']) && trim($s['obra_nombre']) !== '' ? trim($s['obra_nombre']) : 'Sin proyecto',
        'ubicacion' => isset($s['ubicacion_valor']) ? $s['ubicacion_valor'] : '',
        'categoria' => isset($s['categoria']) ? $s['categoria'] : '',
        'subcategoria' => isset($s['subcategoria']) ? $s['subcategoria'] : '',
        'detalle' => isset($s['detalle']) ? $s['detalle'] : '',
        'dias' => isset($s['dias_disponibles']) ? $s['dias_disponibles'] : '',
        'estado' => isset($s['estado']) ? $s['estado'] : 'pendiente',
        'urgencia' => isset($s['urgencia']) ? $s['urgencia'] : 0
    );
}
...
<style>
    .dashboard2-page {
        background: linear-gradient(135deg, #f3f5f2 0%, #f8faf7 48%, #edf2ec 100%);
        min-height: 100vh;
    }
    ...
    .dashboard2-kpi {
        ...
        border-left: 4px solid #608418;
        border-radius: 6px;
        padding: 18px 16px;
        box-shadow: 0 3px 10px rgba(32, 48, 37, .055);
        transition: transform .18s ease, box-shadow .18s ease;
    }

    .dashboard2-kpi:hover {
        transform: translateY(-2px);
        box-shadow: 0 7px 16px rgba(32, 48, 37, .09);
    }

    .dashboard2-kpi:nth-child(2) {
        border-left-color: #e39b32;
    }

    .dashboard2-kpi:nth-child(3) {
        border-left-color: #3d86a8;
    }

    .dashboard2-kpi:nth-child(4) {
        border-left-color: #2e936f;
    }

    .dashboard2-kpi:nth-child(5) {
        border-left-color: #c95b6b;
    }

    .dashboard2-kpi-content {
        display: flex;
        align-items: center;
        gap: 12px;
        min-width: 0;
    ...
    .dashboard2-kpi:nth-child(2) .dashboard2-kpi-icon {
        background: #fff0d5;
        color: #c27b16;
    }

    .dashboard2-kpi:nth-child(3) .dashboard2-kpi-icon {
        background: #e1f0f5;
        color: #3d86a8;
    }

    .dashboard2-kpi:nth-child(4) .dashboard2-kpi-icon {
        background: #dff2ea;
        color: #2e936f;
    }

    .dashboard2-kpi:nth-child(5) .dashboard2-kpi-icon {
        background: #f8e1e5;
        color: #c95b6b;
    }
    ...
    .dashboard2-chart-layout .dashboard2-chart {
        height: 300px;
        min-height: 0;
        padding: 8px 0 8px 8px;
    }

    .dashboard2-chart-side {
        min-width: 0;
        border-left: 1px solid #e8ece7;
        padding: 4px 0 4px 18px;
    }

    .dashboard2-chart-side-title {
        color: #7b857d;
        font-size: .68rem;
        text-transform: uppercase;
        letter-spacing: .05em;
    ...
    .dashboard2-chart-stat:last-child {
        border-bottom: 0;
    }

    .dashboard2-chart-dot {
        width: 9px;
        height: 9px;
        flex: 0 0 9px;
        border-radius: 50%;
    ...
    .dashboard2-chart-stat-value {
        color: #26342b;
        font-size: .78rem;
        font-weight: 700;
    }

    .dashboard2-chart-stat-percent {
        display: block;
        color: #89928b;
        font-size: .66rem;
        margin-top: 2px;
    ...
    .dashboard2-category-summary {
        display: grid;
        gap: 12px;
    }

    .dashboard2-category-item {
        padding: 9px 10px;
        background: #f7f9f6;
        border-left: 3px solid #608418;
    }

    .dashboard2-category-item:nth-child(2) {
        border-left-color: #3d86a8;
    }

    .dashboard2-category-item:nth-child(3) {
        border-left-color: #c95b6b;
    }

    .dashboard2-category-name {
        color: #344238;
        font-size: .72rem;
        font-weight: 700;
    }

    .dashboard2-category-total {
        color: #26342b;
        font-size: 1.15rem;
        font-weight: 700;
        margin-top: 3px;
    ...
    .dashboard2-category-meta {
        color: #7b857d;
        font-size: .66rem;
        margin-top: 3px;
    }

    .dashboard2-table-wrap {
        overflow-x: auto;
    }
    ...
    .dashboard2-table tbody tr:nth-child(even) {
        background: #fcfdfb;
    }
    ...
    .dashboard2-table tr:last-child td {
        border-bottom: 0;
    }
    ...
    .dashboard2-status-pendiente {
        background: #fff0d5;
        color: #926018;
    }

    .dashboard2-status-aprobado {
        background: #dff2ea;
        color: #237454;
    }

    .dashboard2-status-resuelto {
        background: #d9f1e3;
        color: #237044;
    }

    .dashboard2-status-no_corresponde {
        background: #f8e1e5;
        color: #a13f50;
    }
    ...
    .dashboard2-filters input,
    .dashboard2-filters select {
        min-width: 180px;
        padding: 8px 10px;
    ...
    .dashboard2-project-select:hover {
        background: #fff;
    }

    .dashboard2-project-select:focus {
        outline: none;
        border-color: #608418;
        background: #fff;
    }

    .dashboard2-requests-table tbody tr:hover {
        background: #edf5e9;
    }
    ...
    .dashboard2-requests-table .dashboard2-action {
        color: #608418;
        font-weight: 700;
        text-decoration: none;
        white-space: nowrap;
    ...
    .dashboard2-action+.dashboard2-action {
        margin-left: 10px;
    }
    ...
    @media (max-width:800px) {
        ...
        .dashboard2-header-filters {
            width: 100%;
            margin-top: 10px;
        }

        .dashboard2-header-filters select {
            width: 100%;
        }
    }
...
<div class="dashboard2-page">
    <div class="dashboard2-container">
            ...
            <div class="dashboard2-header-filters">
                <select id="dashboard2ProjectFilter" class="dashboard2-project-select">
                    ...
                    <?php foreach (array_keys($porProyecto) as $nombreProyecto): ?>
                        <option value="<?php echo htmlspecialchars($nombreProyecto); ?>">
                            <?php echo htmlspecialchars($nombreProyecto); ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
        </div>
        ...
        <div class="dashboard2-kpis">
            <div class="dashboard2-kpi">
                <div class="dashboard2-kpi-content">
                    <div class="dashboard2-kpi-icon" aria-hidden="true"><i class="fas fa-clipboard-list"></i></div>
                    <div>
                        <div class="dashboard2-kpi-label">Solicitudes totales</div>
                        <div class="dashboard2-kpi-value"><?php echo $totalSolicitudes; ?></div>
                    </div>
                </div>
            </div>
            <div class="dashboard2-kpi">
                <div class="dashboard2-kpi-content">
                    <div class="dashboard2-kpi-icon" aria-hidden="true"><i class="fas fa-clock"></i></div>
                    <div>
                        <div class="dashboard2-kpi-label">Sin acción</div>
                        <div class="dashboard2-kpi-value"><?php echo $totalPendientes; ?></div>
                    </div>
                </div>
            </div>
            <div class="dashboard2-kpi">
                <div class="dashboard2-kpi-content">
                    <div class="dashboard2-kpi-icon" aria-hidden="true"><i class="fas fa-spinner"></i></div>
                    <div>
                        <div class="dashboard2-kpi-label">En gestión</div>
                        <div class="dashboard2-kpi-value"><?php echo $totalEnGestion; ?></div>
                    </div>
                </div>
            </div>
            <div class="dashboard2-kpi">
                <div class="dashboard2-kpi-content">
                    <div class="dashboard2-kpi-icon" aria-hidden="true"><i class="fas fa-check-circle"></i></div>
                    <div>
                        <div class="dashboard2-kpi-label">Resueltas</div>
                        <div class="dashboard2-kpi-value"><?php echo $totalResueltos; ?></div>
                    </div>
                </div>
            </div>
            <div class="dashboard2-kpi">
                <div class="dashboard2-kpi-content">
                    <div class="dashboard2-kpi-icon" aria-hidden="true"><i class="fas fa-times-circle"></i></div>
                    <div>
                        <div class="dashboard2-kpi-label">Rechazadas</div>
                        <div class="dashboard2-kpi-value"><?php echo $totalRechazados; ?></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="dashboard2-grid">
            <section class="dashboard2-panel">
                <div class="dashboard2-panel-header">
                    <h2>Estado general de solicitudes</h2>
                    <p>Distribución de todos los casos registrados</p>
                </div>
                        ...
                        <div class="dashboard2-chart-side">
                            ...
                            <?php foreach ($estados as $estadoClave => $estadoNombre):
                                ...
                                <div class="dashboard2-chart-stat"
                                    data-estado-key="<?php echo htmlspecialchars($estadoClave); ?>">
                                    <span class="dashboard2-chart-dot"
                                        style="background:<?php echo isset($coloresEstado[$estadoClave]) ? $coloresEstado[$estadoClave] : '#608418'; ?>;"></span>
                                    <span
                                        class="dashboard2-chart-stat-label"><?php echo htmlspecialchars($estadoNombre); ?><span
                                            class="dashboard2-chart-stat-percent"><?php echo number_format($porcentajeEstado, 1, ',', '.'); ?>%</span></span>
                                    <span
                                        class="dashboard2-chart-stat-value"><?php echo $estadoTotals[$estadoClave]; ?></span>
                                </div>
                            <?php endforeach; ?>
                        </div>
                    </div>
            ...
            <section class="dashboard2-panel">
                <div class="dashboard2-panel-header">
                    <h2>Estado por tipo de falla</h2>
                    <p>Comparación según la categoría de solicitud</p>
                </div>
                <div class="dashboard2-chart">
                    <div class="dashboard2-chart-layout">
                            ...
                            <div class="dashboard2-category-summary">
                                <?php foreach ($porCategoriaEstado as $categoriaNombre => $valoresCategoria):
                                    ...
                                    <div class="dashboard2-category-item"
                                        data-categoria-name="<?php echo htmlspecialchars($categoriaNombre); ?>">
                                        <div class="dashboard2-category-name">
                                            <?php echo htmlspecialchars($categoriaNombre); ?></div>
                                        <div class="dashboard2-category-total"><?php echo $totalCategoria; ?> casos</div>
                                        <div class="dashboard2-category-meta">
                                            <?php echo number_format($porcentajeCategoria, 1, ',', '.'); ?>% del total ·
                                            <?php echo $abiertosCategoria; ?> abiertos · <?php echo $cerradosCategoria; ?>
                                            cerrados</div>
                                    </div>
                                <?php endforeach; ?>
                            </div>
            ...
            <section class="dashboard2-panel dashboard2-panel-wide">
                <div class="dashboard2-panel-header">
                    ...
                    <p>Volumen, casos pendientes, en gestión, resueltos y rechazados</p>
                </div>
                    ...
                    <table class="dashboard2-table">
                        ...
                        <tbody>
                            <?php if (empty($porProyecto)): ?>
                                <tr>
                                    <td colspan="6" class="dashboard2-empty">No hay datos de proyectos disponibles.</td>
                                </tr>
                            <?php else:
                                foreach ($porProyecto as $proyecto => $indicadores): ?>
                                    <tr>
                                        <td><strong><?php echo htmlspecialchars($proyecto); ?></strong></td>
                                        <td class="dashboard2-number"><?php echo $indicadores['total']; ?></td>
                                        <td><?php echo $indicadores['pendientes']; ?></td>
                                        <td><?php echo $indicadores['en_gestion']; ?></td>
                                        <td><?php echo $indicadores['resueltos']; ?></td>
                                        <td><?php echo $indicadores['rechazados']; ?></td>
                                    </tr>
            ...
            <section class="dashboard2-panel dashboard2-panel-wide">
                <div class="dashboard2-panel-header">
                    <h2>Pendientes por proyecto</h2>
                    <p>Antigüedad de casos abiertos y tiempo promedio de respuesta de casos cerrados</p>
                </div>
                <div class="dashboard2-table-wrap">
                    <table class="dashboard2-table">
                        <thead>
                            <tr>
                                ...
                                <th>Tiempo promedio de casos abiertos</th>
                                <th>Caso pendiente más antiguo</th>
                                <th>Tiempo promedio de respuesta casos cerrados</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php if (empty($pendientesPorProyecto)): ?>
                                <tr>
                                    <td colspan="5" class="dashboard2-empty">No hay datos de proyectos disponibles.</td>
                                </tr>
                            <?php else:
                                foreach ($pendientesPorProyecto as $proyecto => $indicadores): ?>
                                    <tr>
                                        ...
                                        <td><?php echo $indicadores['promedio_abiertos'] === null ? '—' : number_format($indicadores['promedio_abiertos'], 1, ',', '.') . ' días'; ?>
                                        </td>
                                        <td><?php echo $indicadores['caso_antiguo_fecha'] === null ? '—' : number_format(max(0, (time() - $indicadores['caso_antiguo_fecha']) / 86400), 1, ',', '.') . ' días (#' . (int) $indicadores['caso_antiguo_id'] . ')'; ?>
                                        </td>
                                        <td><?php echo $indicadores['promedio_cerrados'] === null ? '—' : number_format($indicadores['promedio_cerrados'], 1, ',', '.') . ' días'; ?>
                                        </td>
                                    </tr>
                                <?php endforeach; endif; ?>
                        </tbody>
            ...
            <section class="dashboard2-panel dashboard2-panel-wide">
                <div class="dashboard2-panel-header">
                    <h2>Solicitudes más antiguas sin acción</h2>
                    <p>Casos pendientes ordenados desde el más antiguo. Se considera sin acción un caso en estado
                        pendiente.</p>
                </div>
                <div class="dashboard2-table-wrap">
                    <table class="dashboard2-table">
                        ...
                        <tbody>
                            <?php if (empty($pendientesAntiguos)): ?>
                                <tr>
                                    <td colspan="7" class="dashboard2-empty">No hay solicitudes pendientes sin acción.</td>
                                </tr>
                            <?php else:
                                foreach ($pendientesAntiguos as $pendiente):
                                    ...
                                    ?>
                                    <tr class="dashboard2-antiguos-row"
                                        data-request-project="<?php echo htmlspecialchars($nombreProyecto); ?>">
                                        ...
                                        <td><?php echo htmlspecialchars(isset($pendiente['nombre']) ? $pendiente['nombre'] : '—'); ?>
                                        </td>
                                        <td><?php echo htmlspecialchars(isset($pendiente['categoria']) ? $pendiente['categoria'] : '—'); ?>
                                        </td>
                                        <td><span class="dashboard2-badge"><?php echo $diasAntiguedad; ?> días</span></td>
                                        <td><a class="dashboard2-action"
                                                href="admin.php?caso=<?php echo (int) $pendiente['id']; ?>"><i
                                                    class="fas fa-cogs"></i> Gestionar</a></td>
                                    </tr>
                                <?php endforeach; endif; ?>
                        </tbody>
                    </table>
            ...
            <section class="dashboard2-panel dashboard2-panel-wide">
                <div class="dashboard2-panel-header" style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px;">
                    <div>
                        <h2>Todas las solicitudes</h2>
                        <p>Listado completo de solicitudes recibidas, con acceso al detalle de cada caso.</p>
                    </div>
                    <div>
                        <button type="button" class="btn btn-sm btn-outline" id="dashboard2ExportBtn" style="cursor:pointer; display:inline-flex; align-items:center; gap:6px; font-weight:600; padding:7px 14px; border-radius:4px;">
                            <i class="fas fa-download"></i> Exportar
                        </button>
                    </div>
                </div>
                <div class="dashboard2-filters">
                    <input type="text" id="dashboard2RequestSearch"
                        placeholder="Buscar por caso, cliente o categoría...">
                    <select id="dashboard2RequestStatus">
                        <option value="">Todos los estados</option>
                        <?php foreach ($estados as $estadoClave => $estadoNombre): ?>
                            <option value="<?php echo htmlspecialchars($estadoClave); ?>">
                                <?php echo htmlspecialchars($estadoNombre); ?></option>
                        <?php endforeach; ?>
                    </select>
                    ...
                    <table class="dashboard2-table dashboard2-requests-table">
                        ...
                        <tbody>
                            <?php foreach ($solicitudes as $solicitud):
                                ...
                                ?>
                                <tr class="dashboard2-request-row"
                                    data-case-id="<?php echo (int) $solicitud['id']; ?>"
                                    data-request-status="<?php echo htmlspecialchars($estadoSolicitud); ?>"
                                    data-request-project="<?php echo htmlspecialchars($proyectoSolicitud); ?>">
                                    <td class="dashboard2-number">#<?php echo (int) $solicitud['id']; ?></td>
                                    <td><?php echo date('d/m/Y', strtotime($solicitud['created_at'])); ?></td>
                                    <td><?php echo htmlspecialchars(isset($solicitud['nombre']) ? $solicitud['nombre'] : '—'); ?>
                                    </td>
                                    <td><?php echo htmlspecialchars($rolSolicitud); ?></td>
                                    <td><?php echo htmlspecialchars(isset($solicitud['categoria']) ? $solicitud['categoria'] : '—'); ?>
                                    </td>
                                    <td><?php echo htmlspecialchars(isset($solicitud['subcategoria']) ? $solicitud['subcategoria'] : '—'); ?>
                                    </td>
                                    <td><?php echo htmlspecialchars(isset($solicitud['ubicacion_valor']) ? $solicitud['ubicacion_valor'] : '—'); ?>
                                    </td>
                                    <td><?php echo htmlspecialchars($proyectoSolicitud); ?></td>
                                    <td><span
                                            class="dashboard2-badge dashboard2-status dashboard2-status-<?php echo htmlspecialchars($estadoSolicitud); ?>"><?php echo htmlspecialchars($estadoNombre); ?></span>
                                    </td>
                                    <td>
                                        <a class="dashboard2-action"
                                            href="admin.php?caso=<?php echo (int) $solicitud['id']; ?>"><i
                                                class="fas fa-cogs"></i> Gestionar</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                            <?php if (empty($solicitudes)): ?>
                                <tr>
                                    <td colspan="10" class="dashboard2-empty">No hay solicitudes registradas.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
...
<script>
window.dashboard2SolicitudesData = <?php echo json_encode($solicitudesExport, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_UNESCAPED_UNICODE); ?>;

// Evento para exportar a Excel
document.getElementById('dashboard2ExportBtn').addEventListener('click', function(e) {
    e.preventDefault();
    ...
    try {
        var visibleCaseIds = [];
        var totalRows = document.querySelectorAll('.dashboard2-request-row').length;
        var visibleRows = document.querySelectorAll('.dashboard2-request-row:not([style*="display: none"])');
        ...
        if (totalRows > 0 && visibleRows.length === 0) {
            if (typeof showToast === 'function') {
                ...
                alert('No hay solicitudes visibles que coincidan con los filtros actuales.');
            }
            btn.disabled = false;
        ...
        var allData = window.dashboard2SolicitudesData || [];
        var exportRows = [];
        ...
        if (allData.length > 0) {
            ...
            visibleRows.forEach(function(row) {
                ...
                if (cells.length >= 9) {
                    exportRows.push({
                        'N° Caso': cells[0].textContent.trim(),
                        'Fecha Ingreso': cells[1].textContent.trim(),
                        'Cliente': cells[2].textContent.trim(),
                        'Rol Solicitante': cells[3].textContent.trim(),
                        'Categoría': cells[4].textContent.trim(),
                        'Subcategoría': cells[5].textContent.trim(),
                        'Ubicación': cells[6].textContent.trim(),
                        'Proyecto / Obra': cells[7].textContent.trim(),
